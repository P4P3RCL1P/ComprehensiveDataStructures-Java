...
public class HashTableArray extends AbstractHashTable{
	...
	//Linked list array for storing hash table
	...
	{
		hashTable = new HashTableNode[INIT_TABLE_SIZE];
		numElements = 0;
	 ...
	 */
	public HashTableArray(final int CUSTOM_TABLE_SIZE)
	{
		hashTable = new HashTableNode[CUSTOM_TABLE_SIZE];
		numElements = 0;
	...
	public boolean contains(Object key, int value) {
		...
		HashTableNode values = hashTable[index];
		while(values != null)
		{
	...
	{
		hashTableSize = hashTable.length*2;

		//double hash table for when we reach 3/4ths the size of our size
		HashTableNode[] largerHashTable = new HashTableNode[hashTableSize];
		//Traverse through current hash table
		for (int i= 0; i < hashTable.length; i++)
		{
			HashTableNode current = hashTable[i];
			
			while(current != null)
			{
				//Generate a new index based on the new hash table size (simply doubled from previous size)
				int newIndex = hashFunction(hashTable[i].getKey());
				
				largerHashTable[newIndex] = hashTable[i];
	...
	public boolean add(Object key, int value) {
		...
		while(current != null)
		{
			...
			else if(!current.getKey().equals(key))
			{
				...
				hashTable[index].setNext(insertNode);
				//hashTable[index].setValue(value);
				return true;
			}
			current = current.getNext();
	...
	public HashTableNode remove(Object key) {
		...
		else
		{
			...
			if(hashTable[deleteIndex].getKey() == key)
			{
				//check if there are chained elements
				if(hashTable[deleteIndex].getNext() != null)
				{
					returnVal = hashTable[deleteIndex];
					hashTable[deleteIndex] = hashTable[deleteIndex].getNext();
					numElements--;
					return returnVal;
				}
				//there are no chains
			...
			{
				HashTableNode previous = hashTable[deleteIndex];
	...
	{
		HashTableArray table = new HashTableArray();
