...
/*
 * Implementation of generic max heap tree. Users can instantiate with any data type.
...
public class MaxHeapTreeGeneric <T extends Comparable<T>> extends AbstractHeapTreeGeneric <T> {
	...
	{	
		...
		heapAr = (HeapTreeNodeGeneric<T>[]) new HeapTreeNodeGeneric[MAXSIZE];
	 ...
	 * @param maxSize
	 */
	public MaxHeapTreeGeneric(int maxSize)
	{	
		root = EMPTY;
		...
		heapAr = (HeapTreeNodeGeneric<T>[]) new HeapTreeNodeGeneric[size];
		treeStack = new StackArrayGeneric<HeapTreeNodeGeneric<T>>();
		treeQueue = new QueueArrayGeneric<HeapTreeNodeGeneric<T>>();
	}
	 ...
	 * @param arrayVal - Checks the value of an index in the array
	 * @return True if the value passed is a string
	 */
	 ...
	 */
	public void heapify (HeapTreeNodeGeneric<T> root)
	{
		...
		HeapTreeNodeGeneric<T> swapNode; //swapNode to point to the node to swap
		...
		swapNode = heapAr[i];
			...
			else // height of tree is greater than 2 (or node count is greater than 3)
			{
				...
				while(i>0) // make sure we aren't going out of bounds of heapAr[i]
				{
					...
					{
						if(swapNode.getRight().getData().compareTo(swapNode.getData()) > 0)
						{
						...
						else if(swapNode.getLeft().getData().compareTo(swapNode.getData()) > 0 )
						{
							swapNode = heapAr[ROOT].getLeft();
							tempP = heapAr[ROOT].getData();
							heapAr[ROOT].setData(swapNode.getData());
	 ...
	 */
	public void heapifyDown(HeapTreeNodeGeneric<T> deleteNode)
	{
		T temp = deleteNode.getData(); //placeholder variable
		int i = ROOT; //beginning at i=1
		HeapTreeNodeGeneric<T> leftNode = deleteNode.getLeft(); //for readability we set leftNode and rightNode to the left and right attributes of deleteNode
		HeapTreeNodeGeneric<T> rightNode = deleteNode.getRight();
	...
	public boolean add(T value)
	{
		...
		{
			HeapTreeNodeGeneric<T> newNode = new HeapTreeNodeGeneric<T>(value); // create new node with data value that is to be inserted
	...
	public T remove(T value)
	{
		...
		if(!getRoot().isEmpty())
		{
			HeapTreeNodeGeneric nodeExists = search(getRoot(), value);
			if(nodeExists == null)
			{
				throw new IllegalStateException();
			}
		...
		HeapTreeNodeGeneric<T> deleteNode = search(getRoot(), value);
		//find the last leaf within the heap tree
		HeapTreeNodeGeneric<T> tempNode = findLastLeaf();
	...
	{
		HeapTreeNodeGeneric<T> location;//variable for wherever we find the heapAr index with matching data values
	...
	public T getLargest()
	{
		...
		HeapTreeNodeGeneric<T> max = heapAr[ROOT]; //assume that heapify rules have been satisfied and that all that is needed is simply fetching the root node
		int i = 2;
	...
	public T getSmallest()
	{
		...
		HeapTreeNodeGeneric<T> min = heapAr[ROOT]; //min set to root node
		int i = 2;
	...
	public Object heapSort()
	{
		...
		HeapTreeNodeGeneric<T> tempTree[]; //used as a copy of heapAr in order to maintain the structure of heapAr after the heapSort method has been called
		tempTree = (HeapTreeNodeGeneric<T>[]) new HeapTreeNodeGeneric[size];
		int j = 0;
	 ...
	 */
	public void printGivenLevel(HeapTreeNodeGeneric<T> root, int level)
	{
	...
	public boolean breadthFirst(HeapTreeNodeGeneric root, T val)
	{
	...
	public boolean depthFirst(HeapTreeNodeGeneric root, T value, boolean valueFound)
	{
	...
	public static void main(String[] args)
	{
		...
		MaxHeapTreeGeneric<Float> exampleHeapTree = new MaxHeapTreeGeneric<Float>();
	
		
		exampleHeapTree.insert(100.1f);
		exampleHeapTree.insert(80.1f);
		exampleHeapTree.insert(120.1f);
		exampleHeapTree.insert(90.1f);
		exampleHeapTree.insert(75.1f);
		exampleHeapTree.insert(110.1f);
		exampleHeapTree.printTree();
		exampleHeapTree.remove(120.1f);
		exampleHeapTree.printTree();
		...
		//exampleHeapTree.insert(1);
		//exampleHeapTree.levelOrder();
		//exampleHeapTree.inOrder(exampleHeapTree.getRoot());
		//System.out.println();
		//exampleHeapTree.preOrder(exampleHeapTree.getRoot());
		//System.out.println();
		//exampleHeapTree.postOrder(exampleHeapTree.getRoot());
		//System.out.println();
