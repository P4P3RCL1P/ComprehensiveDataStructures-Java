 ...
 * @version 1.0
 */
public class MatrixClassGeneric<T> extends AbstractMatrixGeneric<T> {
		/**
	    ...
	    */
	   private MatrixNodeGeneric root;

	   // a matrix must have its height specified
	   // assumes m >= 1 and n >= 1
	   // m is the number of columns (width), n is the number of rows (height)
	   public MatrixClassGeneric(int m, int n) {
	       width = m;
	       height = n;
	       root = new MatrixNodeGeneric();
	       MatrixNodeGeneric currColumn = root;
	       // adds columns. skips one column because root already created
	       for (int i = 1; i < m; i++) {
	    	   MatrixNodeGeneric temp = new MatrixNodeGeneric();
	       ...
	       currColumn = root;
	       for (int i = 0; i < m; i++) {
	    	   MatrixNodeGeneric currRow = currColumn;
	   ...
	   public boolean setData(int m, int n, T value) {
	        ...
	        MatrixNodeGeneric currNode = root;
	        for (int i = 1; i < m; i++) {
	   ...
	   public static void main(String[] args) {
	       ...
	       MatrixClassGeneric test = new MatrixClassGeneric(2, 2);
	       test.setData(1, 1, 1);
	       test.setData(1, 2, 2);
	       test.setData(2, 1, 3);
	       test.setData(2, 2, 4);
